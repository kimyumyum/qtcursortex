...
class MainPage(QMainWindow):
    ...
    def __init__(self):
        ...
        #self.blankFormatAnswersButton.clicked.connect(self.blankFormat)
        self.blankRemoveButton.clicked.connect(self.blankRemove)
        self.answersListWidget.clicked.connect(self.displayAnswerText)

    '''
    blank question tab functions
    '''
    def displayAnswerText(self):
        self.answerText.setText(self.answersListWidget.currentItem().text())
    def blankRemove(self):
        row=  self.answersListWidget.currentRow()
        self.answersListWidget.takeItem(row)
    def addToAnswer(self):
        if self.answersListWidget.count() >4:
            return
        self.answersListWidget.addItem(self.answerText.toPlainText())
